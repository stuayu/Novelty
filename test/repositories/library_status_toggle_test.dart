...
void main() {
  ...
  group('LibraryStatus.toggle', () {
    ...
    setUp(() {
      ...
      when(mockDatabase.watchIsInLibrary(any, any)).thenAnswer(
    ...
    test('なろう同期成功時はnarouSyncFailed:falseで同期済みを記録する', () async {
      ...
      when(mockDatabase.addToLibrary(NovelSource.narou, testNcode)).thenAnswer((_) async => 1);
      ...
      final result = await container
          .read(libraryStatusProvider(NovelSource.narou, testNcode).notifier)
          .toggle(testNovel);
    ...
    test('なろう同期で予期しない例外が発生してもローカル追加はerrorにならない', () async {
      ...
      verify(mockDatabase.addToLibrary(NovelSource.narou, testNcode)).called(1);
      verifyNever(
        mockDatabase.markNarouBookmarkSynced(
          NovelSource.narou,
          testNcode,
    ...
    Future<LibraryStatus> waitForRegisteredState() async {
      final notifier = container.read(
        libraryStatusProvider(NovelSource.narou, testNcode).notifier,
      );
      final completer = Completer<void>();
      final subscription = container.listen(
        libraryStatusProvider(NovelSource.narou, testNcode),
        (previous, next) {
    ...
    test('保存トークンが無くてもncodeを使ってなろう解除を呼ぶ', () async {
      when(mockDatabase.watchIsInLibrary(NovelSource.narou, testNcode)).thenAnswer(
        (_) => Stream.value(true),
      );
      when(mockDatabase.removeFromLibrary(NovelSource.narou, testNcode)).thenAnswer(
        (_) async => 1,
      );
    ...
    test('なろう解除失敗時はnarouSyncFailed:trueでremovedを返す', () async {
      ...
      verifyNever(mockDatabase.removeFromLibrary(NovelSource.narou, testNcode));
    });
  });
}
